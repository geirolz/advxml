...
package object data {
  ...
  /** Represents a function `A => ValidatedThrow[B]` to simplify method and class signatures.
    * Because the conversion can fail the output is wrapped into cats [[ValidatedNelThrow]] in order to handle the errors.
    *
  ...
  /** This is just an alias for [[advxml.core.data.ValidatedConverter]] parametrized with `NodeSeq` on right side.
    ...
    */
  @implicitNotFound(
    """Missing implicit XmlDecoder instance to covert ${T} into ValidatedNelThrow[NodeSeq]."""
  )
